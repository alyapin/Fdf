...
/*   Updated: 2019/08/03 19:57:25 by kzina            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_color( int start, int end, int current, t_linecor *col)
{
    ...
    red = get_light((col->color0 >> 16) & 0xFF, (col->color1 >> 16) & 0xFF, percentage);
    green = get_light((col->color0 >> 8) & 0xFF, (col->color1 >> 8) & 0xFF, percentage);
...
void    draw_line(t_line *line, int x1, t_mlx *params, t_linecor *xy)
{
    ...
    while (x <= x1)
    {
        if (x > WIDTH || x < 0 || y > HEIGH || y < 0)
            return ;
        param->data_address[(k == 1 ? y : x) * param->line_size + (k == 1 ? x : y)] = get_color(start, x1, x, xy);
        line->error -= line->dy;
        if (line->error < 0)
        {
            y +=line->ystep;
